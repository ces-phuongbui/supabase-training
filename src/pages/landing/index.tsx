...
const Feature = ({ title, description, icon }: { title: string; description: string; icon: React.ReactNode }) => {
    return (
        <Box display="flex" gap={2} maxWidth={400}>
            <Box fontSize={64} display="flex" alignItems="flex-start" justifyContent="center">
                {icon}
            </Box>
            <Box>
...
export default function LandingPage() {
    ...
    return (
        <Stack>
            <Box component="header" p={2} display="flex" justifyContent="space-between">
                <AppIcon />
                <Stack direction="row" gap={2}>
                    {user ? (
                        <Button variant="contained" component={Link} to="/requests" size="small">
                            Dashboard
                        </Button>
                    ) : (
                        <>
            ...
            <Box>
                <Box pb={{ xs: 5, sm: 20, md: 25, lg: 30 }}>
                    <Box px={4} py={10} maxWidth={1280} mx="auto">
                        ...
                        >
                            Create customizable RSVP quickly and send them online. No more manually counting cards.
                            Easily determine number of attendees.
                        </Typography>
